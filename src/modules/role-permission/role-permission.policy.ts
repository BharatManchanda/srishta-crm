...
import { ROLE_CONFIG_MODULE_ID } from 'src/seeders/module.seeder';
...
export class RolePermissionPolicy {
  ...
  private async hasPermission(
    ...
    const isAllow = await this.paymentsService.isAllowedModules(currentUser.id, ROLE_CONFIG_MODULE_ID);
    if (!isAllow) {
    ...
    const permission = await this.prisma.rolePermission.findFirst({
      where: {
        ...
        moduleId: ROLE_CONFIG_MODULE_ID,
        isAllow: true,
        [action]: true,
      },
    });
  ...
  async authorize(
    ...
    switch (action) {
      ...
      case 'view':
        allowed = await this.canView(currentUser, roleConfId!);
        break;
      ...
      case 'update':
        allowed = await this.canUpdate(currentUser, roleConfId!);
        break;

      case 'delete':
        allowed = await this.canDelete(currentUser, roleConfId!);
        break;
    }
